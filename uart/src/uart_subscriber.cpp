...
public:
  uart_subscriber()
  : base("minimal_subscriber"), _io_context{}, _serial_port{_io_context}
  {
    ...
    _servo_subscription = base::create_subscription<std_msgs::msg::Int16>(
        "/control/steering_angle/target", 10,
        std::bind(&uart_subscriber::_servo_callback, this,
                  std::placeholders::_1));
    _motor_subscription = base::create_subscription<std_msgs::msg::Float32>(
        "/control/velocity/target", 10,
        std::bind(&uart_subscriber::_motor_callback, this,
                  std::placeholders::_1));
...
private:
  auto _servo_callback(std_msgs::msg::Int16::UniquePtr msg) -> void
  {
    ...
    // _serial_port.write_some(boost::asio::buffer(encoded.data(),
    // encoded.size()));
    boost::asio::write(_serial_port,
                       boost::asio::buffer(encoded.data(), encoded.size()));
  }
  ...
  auto _motor_callback(std_msgs::msg::Float32::UniquePtr msg) -> void
  {
    ...
    // _serial_port.write_some(boost::asio::buffer(buffer.data(),
    // buffer.size()));
    boost::asio::write(_serial_port,
                       boost::asio::buffer(encoded.data(), encoded.size()));
  }
